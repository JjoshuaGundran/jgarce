...
export const Navigation = ({ active }: Props) => {
  const activeNav = navItems.find(t => t.id === active);
  const [hoverNav, setHoverNav] = useState<NavItem | undefined>(activeNav);
  
  return (
    <>
      <div className='bg-zinc-700 text-zinc-400 text-center font-medium md:text-lg relative overflow-hidden'>
        <div className={`p-1.5 md:p-2 transition-opacity${hoverNav ? ' opacity-0' : ''}`}>You already know the vibe</div>
        {hoverNav && <div className="p-1.5 md:p-2 absolute top-0 w-full">{hoverNav.tagline}</div>}
      </div>
      <nav id='socials' className='bg-zinc-800 text-xl p-3 gap-3 flex flex-wrap items-center justify-around font-semibold'>
        {navItems.map(i => (
          <a
            ...
            onMouseEnter={() => i.id === activeNav?.id ? undefined : setHoverNav(i)}
            onMouseLeave={() => i.id === activeNav?.id ? undefined : setHoverNav(navItems.find(t => t.id === activeNav?.id))}
          >
            <Icon icon={i.icon} />
            <span>{i.title}</span>
          </a>
